...
// ─────────────────────────────────────────────
const mockCustomer = { id: 1, userId: 10, name: 'Carlos', lastName: 'García', email: 'carlos@test.com' };
...
describe('PaymentService', () => {
  ...
  describe('HU-10 — Pago de Reserva', () => {
    ...
    it('[E1] debe procesar el pago correctamente y actualizar la reserva a booked', async () => {
      ...
      // Act
      const result = await service.createPayment(validPaymentDto);
      ...
      expect(mockBookRepository.update).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          status: 'booked',
          confirmationCode: expect.stringMatching(/^RES-\d{8}-\d{3}$/),
        }),
      );
    ...
    it('[E2] debe lanzar NotFoundException si la reserva no existe', async () => {
      ...
      // Act & Assert
      await expect(service.createPayment(validPaymentDto))
        .rejects
    ...
    it('[E4] debe guardar solo los últimos 4 dígitos de la tarjeta', async () => {
      ...
      const dtoOtraTargeta = { ...validPaymentDto, cardNumber: '5500005555555559' };
      const savedPayment = {
        id: 2, bookId: 1, amount: 1250.00,
        cardLastDigits: '5559', status: 'completed', createdAt: new Date(),
      };
      ...
      // Act
      const result = await service.createPayment(dtoOtraTargeta);
  ...
  describe('HU-11 — Historial de Pagos', () => {
    ...
    it('[E1] debe retornar los pagos del cliente con sus reservas y habitaciones', async () => {
      // Arrange
      const pagos = [
        { id: 1, bookId: 1, amount: 1250, status: 'completed', book: { ...mockPendingBook, room: { code: '102' } } },
        { id: 2, bookId: 2, amount: 750,  status: 'completed', book: { id: 2, roomId: 5,   room: { code: '201' } } },
      ];
      ...
      expect(mockBookRepository.find).toHaveBeenCalledWith({ where: { customerId: mockCustomer.id } });
    ...
    it('[E3] debe lanzar NotFoundException si el userId no tiene cliente', async () => {
      ...
      // Act & Assert
      await expect(service.findByCustomer(99))
        .rejects
        .toThrow(NotFoundException);
    });
  });
  ...
  describe('getVoucher', () => {
    ...
    it('debe retornar el voucher completo de un pago confirmado', async () => {
      ...
      const payment = { amount: 1250, cardLastDigits: '1111', createdAt: new Date() };
