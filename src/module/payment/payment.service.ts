...
export class PaymentService {
  ...
  async createPayment(dto: CreatePaymentDto, userId: number): Promise<Payment> {
    ...
    if (
      ...
      book.customer.userId !== userId
    ) {
      throw new ForbiddenException('No tiene permiso para pagar esta reserva');
    }
    if (book.status !== 'pending') {
      throw new BadRequestException(
        'Esta reserva ya ha sido pagada o procesada',
      );
    }
  ...
  async getVoucher(bookId: number) {
    ...
    if (!book) throw new NotFoundException('Reserva no encontrada');
    if (!book.confirmationCode)
      throw new BadRequestException('Esta reserva no tiene pago registrado');
